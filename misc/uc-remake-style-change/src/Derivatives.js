...
let derivTitles = ['Increase','Growth','Expansion','Peak','BEYOND']
...
let derivProductions = [D(0.5),D(0.3),D(0.2),D(0.1),D(0.05)]
function calculateDerivProductions(){
    derivProductions[0] = theoryEffects[0].times(upgradeEffects[0])
    derivProductions[1] = D(100).times(lostCycleEffects[0]).times(theoryEffects[12]).times(upgradeEffects[1])
    derivProductions[2] = D(100).times(upgradeEffects[2])
    derivProductions[3] = D(100).times(theoryEffects[1]).times(theoryEffects[8]).times(upgradeEffects[3])
    derivProductions[4] = D(100).times(theoryEffects[13]).times(theoryEffects[14])
}
function produceDerivs(diff){
  ...
  /*
    ...
    */
  for(let x=0;x<4;x++){
    data.derivs[x].amt=data.derivs[x].amt.plus(derivProductions[x+1].times(data.derivs[x+1].amt).times(diff))
  }
}
...
let lastDerivHeld = 10
function updateDerivText(i){
  if(lastDerivHeld!=i)derivInfo[0].textContent = `Derivative ${derivNames[i]}: [${derivTitles[i]}]`
  if(i==0){
    derivInfo[1].textContent=`Cost: ${format(data.derivs[0].c)} Oddities`
    derivInfo[2].textContent=`[${format(data.derivs[0].b)}] ${format(data.derivs[0].amt)}`
    derivInfo[3].textContent=`Produces ${format(derivProductions[0])} Oddities [${format(data.oddityGain)}/s]`
  }
  else{
    if(data.derivs[i].u){
      derivInfo[1].textContent=`Cost: ${format(data.derivs[i].c)} Purchased D.${derivNames[i-1]}`
      derivInfo[2].textContent=`[${format(data.derivs[i].b)}] ${format(data.derivs[i].amt)}`
      derivInfo[3].textContent=`Produces ${format(derivProductions[i])} D.${derivNames[i-1]} [${format(data.derivs[i].amt.times(derivProductions[i-1]))}/s]`
    }else{
      if(lastDerivHeld==0){
        derivInfo[2].textContent=""
        derivInfo[3].textContent=""
      }
      if(lastDerivHeld!=i)derivInfo[1].textContent=`Unlock for ${format(derivUnlockCost[i-1])} Oddities`
    }
  }
  lastDerivHeld=i
}