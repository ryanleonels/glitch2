function nerf(num, start, pow, pow2,thing=new Decimal(1)){
  ...
  let s = new Decimal(pow).pow(num.log(start).pow(pow2).sub(1)).min(1).pow(thing).pow(new Decimal(1.5).sub(hasUpgrade("bo",11)?upgradeEffect("bo",11):0))
  player.pointNerf=s.max(challengeCompletions("bo",12)>=3?0.2:0)
...
addLayer("p", {
    ...
    effect(){
      if(inChallenge("s",12)||inChallenge("ac",21))return new Decimal(1)
      if(hasUpgrade("s",24)){
        let r = player.p.points.pow(0.5).plus(1)
        if(hasUpgrade("l",13))r=r.pow(100)
        if(hasUpgrade("l",23))r=r.pow(1.5)
        if(hasUpgrade("l",41))r=r.pow(1.5)
        if(hasUpgrade("l",53))r=r.pow(1.5)
        if(hasUpgrade("l",54))r=r.pow(1.5)
        if(hasUpgrade("l",55))r=r.pow(1.5)
        if(hasUpgrade("l",61))r=r.pow(1.5)
        if(hasUpgrade("l",62))r=r.pow(1.5)
        if(!hasUpgrade("a",21))r=nerf(r,new Decimal("1e500"),0.8,0.5)
        if(hasUpgrade("di",23))r=r.pow(1.5)
        if(hasUpgrade("a",33))r=r.pow(1.5)
        if(hasUpgrade("di",23))r=r.pow(upgradeEffect("l",45))
        if(hasUpgrade("di",33))r=r.pow(1.5)
        if(hasMilestone("ch",4))r=r.pow(1.5)
        if(player.b.clickables[12]==3)r=r.pow(1.5)
        ...
        return r.root(new Decimal(4).sub(challengeCompletions("bo",12)/10))
      }
    ...
    effectDescription(){
      ...
      if(hasUpgrade("s",24))s="multiplying point gain by "+format(this.effect())+(this.effect().gte("1e1e6")?" (softcapped)":this.effect().gte("1e500")&&!hasUpgrade("a",21)?" (nerfed)":"")
      return s
    },
    color: "#964b00",
    ...
    if(hasUpgrade("p",14))r=r.div(Decimal.pow(2,player.p.points).pow(hasUpgrade("p",23)?2:1))
               ...
               return r.div(hasUpgrade("p",25)&&hasUpgrade("nerfs",14)?100:1)
    ...
    upgrades: {
      11: {
        ...
        cost: new Decimal(2),
        unlocked(){return player.p.best.gte(2)},
      },
      12: {
        title: "Nerf 2",
...
addLayer("s", {
  ...
  branches:["p"],
    ...
    if(hasUpgrade("p",12))f=f.sqrt()
             if(hasUpgrade("p",15))f=f.div(player.p.points.plus(1).pow(hasUpgrade("p",22)?2:1))
             return f.pow(hasUpgrade("p",22)?2:1)*/
      if(hasUpgrade("s",14))f=f.sqrt()
      if(inChallenge("s",11))f=f.pow(0.5)
      ...
      return f.root(hasUpgrade("ac",14)?new Decimal(4).sub(upgradeEffect("ac",12)):4)
    },
    effectDescription(){
...
if(hasUpgrade("s",22))r=new Decimal(10)
    ...
    gainMult() { // Calculate the multiplier for main currency from bonuses
      if(inChallenge("s",12)||inChallenge("ac",12))return new Decimal(0)
        mult = new Decimal(1)
      ...
      if(hasUpgrade("l",42))mult=mult.times(player.l.points.plus(1).cbrt())
      ...
      if(hasUpgrade("nerfs",13))mult=mult.times(new Decimal(3).pow(hasUpgrade("ac",15)?upgradeEffect("ac",15):1))
        return mult
    },
    gainExp() { // Calculate the exponent on main currency from bonuses
        return new Decimal(1).plus(hasUpgrade("l",35)?1:0).times(hasUpgrade("di",23)?layers.l.clickables[12].effect():1).mul(hasMilestone("b",8)&&player.b.boost1?1.05:1).add(hasChallenge("ac",12)?0.5:0)
    },
    row: 1, // Row the layer is in on the tree (0 is the first row)
    hotkeys: [
        {key: "s", description: "S: Reset for super prestige points", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
    ],
    layerShown(){return player[this.layer].unlocked||hasUpgrade("p",25)},
    upgrades: {
      11: {
        title: "Super 1",
  ...
  milestones: {
    0: {
        requirementDescription: "3 super prestige points",
        effectDescription: "Keep 2 upgrades per super prestige upgrade",
        done() { return player.s.best.gte(3) }
    },
    1: {
        requirementDescription: "200000 super prestige points",
        effectDescription: "Automatically gain super prestige points",
        done() { return player.s.best.gte(2e5) }
    },
},
  ...
  challenges: {
    11: {
        name: "Based",
        challengeDescription: "Prestige point cost base ^2 and super prestige effect ^0.5",
...
12: {
        ...
        goal(){return new Decimal(inChallenge("bo",11)?"1e666":"1e1000")},
  ...
  something(){
    for(let l in layers.s.upgrades){
      ...
      if(l=="rows"||l=="cols")continue
      if(l>16)continue
      if(layers.s.upgrades[l].unlocked){
      if(hasUpgrade("s",l)&&!hasUpgrade("p",l)){player.p.upgrades.push(Number(l))}
        if(hasUpgrade("s",l)&&!hasUpgrade("p",Number(l)+10)){player.p.upgrades.push(Number(l)+10)}
      }
}
  },
  doReset(l){if(layers[l].row>this.row){
    let keep = []
  ...
  branches:["s","p"],
    ...
    gainMult() { // Calculate the multiplier for main currency from bonuses
      if(inChallenge("s",12))return new Decimal(0)
        mult = new Decimal(1)
      if(hasUpgrade("l",21))mult=mult.mul(Decimal.pow(3,player.l.upgrades.length))
      ...
      if(inChallenge("bo",11))mult=mult.mul(1e5)
      if(challengeCompletions("bo",12)>=5)mult=mult.mul(4e5)
      mult=mult.mul(player.e.sacrifice[0].pow(69420).add(1))
        return mult
    },
    gainExp() { // Calculate the exponent on main currency from bonuses
        return new Decimal(1).mul(hasMilestone("b",8)&&!player.b.boost1?1.05:1)
    },
    row: 1, // Row the layer is in on the tree (0 is the first row)
    hotkeys: [
        {key: "l", description: "L: Reset for layer points", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
    ],
    layerShown(){return player[this.layer].unlocked||hasChallenge("s",11)},
  upgrades: {
      11: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Points",
        description: "Super prestige effect exponent is now 120",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(50))},
        unlocked(){return player.l.unlocked||inChallenge("bo",12)},
      },
    12: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Prestige Points",
        description: "Points are multiplied by layer points, prestige points, and super prestige points",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(300))},
        unlocked(){return hasUpgrade("l",11)||inChallenge("bo",12)},
      },
    13: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Boosters",
        description: "Prestige point effect ^100, gain 2x super prestige points",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(600))},
        unlocked(){return hasUpgrade("l",12)||inChallenge("bo",12)},
      },
    14: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Generators",
        description: "Unlock another super prestige challenge",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(1250))},
        unlocked(){return hasUpgrade("l",13)||inChallenge("bo",12)},
      },
    15: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Time capsules",
        description: "Unlock a new layer effect",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(3000))},
        unlocked(){return hasChallenge("s",12)||inChallenge("bo",12)},
      },
    21: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Enhancer points",
        description: "Each upgrade triples layer gain",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(6000))},
        unlocked(){return hasUpgrade("l",15)||inChallenge("bo",12)},
      },
    22: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Space energy",
        description: "Divide layer gain by 1.25",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(5e6))},
        unlocked(){return hasUpgrade("l",21)||inChallenge("bo",12)},
      },
    23: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Super boosters",
        description: "Boosters are 2x stronger",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(1e7))},
        unlocked(){return hasUpgrade("l",22)||inChallenge("bo",12)},
      },
    24: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Super generators",
        description: "Generators are 2x stronger",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(1e8))},
        unlocked(){return hasUpgrade("l",23)||inChallenge("bo",12)},
      },
    25: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Hinderance spirits",
        description: "Buff all super prestige challenge rewards",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(1e9))},
        unlocked(){return hasUpgrade("l",24)||inChallenge("bo",12)},
      },
    31: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Quirks",
        description: "Gain more super prestige points based on points",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(5e9))},
        unlocked(){return hasUpgrade("l",25)||inChallenge("bo",12)},
      },
    32: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Subspace",
        description: "Unlock a buyable",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(3e10))},
        unlocked(){return hasUpgrade("l",31)||inChallenge("bo",12)},
      },
    33: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Solarity",
        description: "Super boosters are stronger",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(1e30))},
        unlocked(){return hasUpgrade("l",32)||inChallenge("bo",12)},
      },
    34: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Magic",
        description: "Unlock a spell",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(3e35))},
        unlocked(){return hasUpgrade("l",33)||inChallenge("bo",12)},
      },
    35: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        ...
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(1e42))},
        unlocked(){return hasUpgrade("l",34)||inChallenge("bo",12)},
      },
    41: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Phantom Souls",
        description: "P and S layer effects are stronger",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(5e54))},
        unlocked(){return hasUpgrade("l",35)||inChallenge("bo",12)},
      },
    42: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Honour",
        description: "cbrt(layer points) multiply super prestige point gain",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(1e60))},
        unlocked(){return hasUpgrade("l",41)||inChallenge("bo",12)},
      },
    43: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Nebula",
        description: "Point gain nerf is 20% weaker",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(4.2e69))},
        unlocked(){return hasUpgrade("l",42)||inChallenge("bo",12)},
      },
    44: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        ...
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(1e81))},
        unlocked(){return hasUpgrade("l",43)||inChallenge("bo",12)},
      },
    45: {canAfford(){return (player[this.layer].points.gte(this.cost()))&&(player.l.upgrades.length<challengeCompletions("bo",12)+1||!inChallenge("bo",12))},
        title: "Imperium",
        description: "Point gain nerf is affected by the layer effect at a reduced rate, and unlock the Aarex layer",
        cost(){return (inChallenge("bo",12)?new Decimal(0):new Decimal(1e100))},
        unlocked(){return hasUpgrade("l",44)||inChallenge("bo",12)},
    ...
    51: {
        ...
        unlocked(){return hasChallenge("a",11)&&!inChallenge("bo",12)},
      },
    52: {
        title: "Gears",
        description: "Unlock Timewalls. (subtab in Aarex Timewalls layer)",
        cost: new Decimal("6.9e420"),
        unlocked(){return hasUpgrade("l",51)&&!inChallenge("bo",12)},
      },
    53: {
        title: "Machines",
        ...
        unlocked(){return hasUpgrade("a",24)&&!inChallenge("bo",12)},
      },
    54: {
        title: "Energy",
    ...
    62: {
        ...
        cost: new Decimal("1e1242"),
        unlocked(){return hasUpgrade("l",61)},
      },
  },
  passiveGeneration(){return hasUpgrade("l",51)||hasChallenge("bo",12)},
  buyables: {
    11: {
      ...
      effect(){let e = new Decimal(5).pow(getBuyableAmount(this.layer,this.id))
    ...
    12: {
        cost() { return new Decimal(1e79).mul(Decimal.pow(10,getBuyableAmount(this.layer, this.id).pow(1.5))) },
        display() { return "Multiply super prestige point gain. Currently: x"+format(this.effect())+"\nCost:"+format(this.cost()) },
      effect(){return new Decimal(1e4).plus(hasUpgrade("a",11)?(new Decimal(player.a.upgrades.length).pow(2).times(10000)):0).pow(getBuyableAmount(this.layer,this.id)).pow(player.b.clickables[11]==1||challengeCompletions("b",11)>=1?100:1)},
        canAfford() { return player[this.layer].points.gte(this.cost()) },
        buy() {
            player[this.layer].points = player[this.layer].points.sub(this.cost())
            setBuyableAmount(this.layer, this.id, getBuyableAmount(this.layer, this.id).add(1))
...
13: {
        cost() { return new Decimal("1e555").mul(Decimal.pow(10,getBuyableAmount(this.layer, this.id).pow(1.6))) },
  ...
  update(diff){
    if(hasMilestone("b",2)){
      ...
      if(tmp.l.buyables[12].canAfford)setBuyableAmount("l",12,player.l.points.div(1e79).log10().root(1.5).floor().add(1))
      if(tmp.l.buyables[13].canAfford)setBuyableAmount("l",13,player.l.points.div("1e555").log10().root(1.6).floor().add(1))
    }
    if(!hasUpgrade("l",62)){
  ...
  clickables: {
    ...
    12: {
      ...
      onClick(){setClickableState("l",12,1)
        ...
        effect(){return new Decimal(1).plus(layers.l.effect().times(layers.a.effect()).div(hasUpgrade("l",62)?50:100))},
      unlocked(){return (hasUpgrade("a",13))}
    },
    13: {
        canClick(){return this.unlocked()},
      onClick(){player.l.upgrades=[]},
        display() {
          
          return "Respec layer upgrades"},
      unlocked(){return (inChallenge("bo",12))}
    },
},
  doReset(l){
...
timewall: new Decimal(0),
  ...
  effect(){return new Decimal(0.6).times(player.a.points.plus(1).pow(0.75)).plus(0.4)},
  effectDescription(){return "Making <b>Imperium</b> "+format(this.effect())+"x stronger"+(
  ...
  branches:["l","p"],
    ...
    requires(){
      ...
      r=r.div(player.e.sacrifice[1].pow(200000).add(1))
               return r
    }, // Can be a function that takes requirement increases into account
  ...
  base: new Decimal(1e3),
    exponent(){
      ...
      return new Decimal(2.4)}, // Prestige currency exponent
    gainMult() { // Calculate the multiplier for main currency from bonuses
      if(inChallenge("s",12))return new Decimal(1)
        mult = new Decimal(1)
    ...
    layerShown(){return player[this.layer].unlocked||hasUpgrade("l",45)},
  autoPrestige(){return hasUpgrade("l",53)},canBuyMax(){return hasMilestone("b",3)},
challenges: {
    11: {
        name: "Timewalled",
        challengeDescription: "Point gain is divided by e5800",
        goal: new Decimal(1),
        rewardDescription(){return "Point gain is multiplied by e5800 after nerf"},
      unlocked(){return player.a.best.gte(4)},
    },

},
  upgrades: {
      11: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "T",
    ...
    12: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "I",
    ...
    13: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "M",
    ...
    14: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "E",
    ...
    21: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "W",
    ...
    22: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "A",
    ...
    23: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "L",
    ...
    24: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "L",
    ...
    31: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "G",
    ...
    32: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "A",
    ...
    33: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "M",
    ...
    34: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "I",
    ...
    35: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "N",
    ...
    36: {currencyLayer: "a",currencyInternalName: "timewall",
        title: "G",
  ...
  update(diff){
    if(player.subtabs.a.mainTabs=="Timewalls"&&!layers.a.tabFormat.Timewalls.unlocked()){
      player.subtabs.a.mainTabs="Challenges"
    }
    if(hasUpgrade("l",52)){
      ...
      player.a.timewall=player.a.timewall.root(0.75).add(delta.mul(gain)).pow(0.75)
    }
    if(hasUpgrade("a",14)){
      if(!hasMilestone("b",2)){
      let max=10
  ...
  "Timewalls":{
    ...
    "blank",
      ["display-text",function(){
        let s = "You have "+format(player.a.timewall)+" timewall^0.75."
        return s
      }],
      "blank",
      "blank",
...
velocity: new Decimal(1),
  ...
  resetsNothing(){return true},
    ...
    layerShown(){return player[this.layer].unlocked||hasUpgrade("l",62)},
...
12: {title:"Tier",
  ...
  22: {title:"Crackle",
        cost() { return new Decimal(30).plus(getBuyableAmount(this.layer,this.id).pow(2)) },
  ...
  upgrades: {
      11: {
        ...
        effect(){let eff=(hasUpgrade(this.layer,11)?player.di.points.plus(1).ln().plus(1):new Decimal(1))
        if(hasUpgrade(this.layer,12))eff=eff.pow(tmp.di.upgrades[12].effect)
        return eff},
        eff2(){let eff=(hasUpgrade(this.layer,11)?getBuyableAmount("di",12).plus(1).pow(2):new Decimal(1))
          if(hasUpgrade(this.layer,12))eff=eff.pow(tmp.di.upgrades[12].effect)
        return eff
        },
...
12: {
    ...
    13: {
        ...
        effect(){
          let eff=(hasUpgrade(this.layer,13)?player.di.velocity.plus(1).ln().plus(1):new Decimal(1))
...
if(hasUpgrade(this.layer,21))eff=eff.pow(x)
    ...
    14: {
        title: "Time reversal",
        description(){return "Meters^10 multiplies layer gain, log(points) multiplies time speed. Currently: x"+format(this.effect())+", x"+format(this.eff2())},
        effect(){
          let eff=(hasUpgrade(this.layer,14)?player.di.points.plus(1).pow(10):1)
    ...
    15: {
        ...
        description(){return "Unlock Jerk, which increases acceleration, and unlock a layer"},
    ...
    21: {
        ...
        description(){
          ...
          if(hasMilestone("ch",4)){s+=", ^1.50, ^420.69, *"+format(Decimal.pow(3,getBuyableAmount("ad",22)))+", ^2.00, *"+format(player.xyz.points.times(player.ab.points.pow(10)).times(player.ml.layers).times(player.ss.points.log10().pow(0.1)).times(player.w.points))+", *"+format(Decimal.pow(10,tmp.di.upgrades[21].description.length))+", ^694.20"}
...
if(eff.gt(Number.MAX_VALUE)){eff=eff.pow(0.5).times(new Decimal(Number.MAX_VALUE).sqrt())}
    ...
    33: {
        title: "Derivatives",
        description(){return "Square the prestige effect"},
        cost: new Decimal("1e22111"),
        unlocked(){return (hasUpgrade("ad",35))},
        canAfford(){return player.di.points.gte(this.cost)},
        pay(){player.di.points=player.di.points.sub(this.cost)
    ...
    34: {
        title: "Elementary",
        description(){return "Unlock elementary (a side layer)"},
        cost: new Decimal("1e22146"),
        unlocked(){return (hasUpgrade("di",33))},
        canAfford(){return player.di.points.gte(this.cost)},
        pay(){player.di.points=player.di.points.sub(this.cost)
  ...
  getTimeSpeed(){
    ...
    if(player.b.clickables[12]==1)s=s.pow(1.05)
    if(hasChallenge("cc",11))s=s.mul(Decimal.pow(1000,Decimal.pow(player.cc.challenges[11],1.5)))
    s=s.mul(buyableEffect("db",11))
    s=s.mul(player.e.sacrifice[3].pow(100).add(1))
    return s.max(1)
  },
  update(diff){
    ...
    if(hasUpgrade("di",24)&&layers.di.buyables[22].canAfford()){
      let pogger = getBuyableAmount("di",21).sub(30).max(0).pow(0.5).floor().plus(1)
    ...
    let thing = player.di.velocity.times(tmp.di.upgrades[11].effect).times(hasUpgrade("ad",13)?player.ad.points.max(1):1).times(hasUpgrade("ad",32)?layers.di.getTimeSpeed():1).pow(inChallenge("c",11)?0.5:1).pow(hasChallenge("c",11)?1.1:1).pow(hasChallenge("c",41)?1.1:1).times(hasUpgrade("d",41)?layers.ad.effect():1).times(hasMilestone("c",4)?Decimal.pow(3,getBuyableAmount("ad",22)):1)
    if(hasUpgrade("nerfs",21))thing=thing.mul(player.di.best.pow(0.1).max(1))
    thing=thing.pow(0.825)
    if(thing.gt(hasChallenge("c",31)?"e1010":"e1000"))thing=thing.pow(0.825).mul(hasChallenge("c",31)?new Decimal("1e1010").pow(0.175):1e175)
    player.di.points=player.di.points.plus(thing.mul(diff))
    if(player.di.best.lt(player.di.points)){
      player.di.best=player.di.points
    }
  },
  tabFormat: [
...
effect(){let f= player.ad.best.max(10).pow(10)
...
return f},
  ...
  prestigeButtonText(){return  "Unlock Antimatter Dimensions"},
    ...
    layerShown(){return player[this.layer].unlocked||hasUpgrade("di",23)},
buyables: {
...
12: {title:"2nd dimension",
  ...
  13: {title:"3rd dimension",
        ...
        display() { return "Gain 1 2nd dimension per second. \nCurrently: "+format(player.ad.d3)+"/s\n\nCost: "+format(this.cost())+" antimatter\n\nYou have "+format(getBuyableAmount(this.layer,this.id).sqrt())+" bought 3rd dimensions"},
        canAfford() { return player[this.layer].points.gte(this.cost())&&(!inChallenge("c",12)||getBuyableAmount(this.layer, this.id).lt(100)) },
        buy() {
    ...
    21: {title:"4th dimension",
        cost() { return new Decimal(1e32).mul(Decimal.pow(1000,getBuyableAmount(this.layer, this.id))) },
  ...
  upgrades: {
    ...
    12: {
        ...
        effect(){return layers.di.getTimeSpeed().pow(0.02)},
    ...
    13: {
        ...
        unlocked(){return (player.cc.challenges[12]>=2||hasUpgrade(this.layer,this.id))},
        canAfford(){return player.ad.points.gte(this.cost)},
        pay(){player.ad.points=player.ad.points.sub(this.cost)
    ...
    14: {
        ...
        unlocked(){return (hasUpgrade(this.layer,13)||hasUpgrade(this.layer,this.id))},
        canAfford(){return player.ad.points.gte(this.cost)},
        pay(){player.ad.points=player.ad.points.sub(this.cost)
    ...
    22: {
      ...
      effect(){return player.ad.points.plus(1).log10().plus(1).times(player.timePlayed)},
    ...
    23: {
        ...
        unlocked(){return (getBuyableAmount(this.layer,32).gte(5)||hasUpgrade(this.layer,this.id))},
        canAfford(){return player.ad.points.gte(this.cost)},
        pay(){player.ad.points=player.ad.points.sub(this.cost)
    ...
    25: {
        title: "Infinity",
        description(){return "Unlock an Antimatter buyable, 12th nerf's 1st effect is now 0.85"},
    ...
    31: {
        title: "Replicanti",
        description(){return "Unlock 6 timewall upgrades, tickspeed cost scaling is slower"},

        cost: new Decimal("1e1431"),
        unlocked(){return (hasUpgrade(this.layer,25))},
        canAfford(){return player.ad.points.gte(this.cost)},
        pay(){player.ad.points=player.ad.points.sub(this.cost)
  ...
  automateStuff(){
    if(hasMilestone("b",4)){
      ...
      if(layers.ad.buyables[21].canAfford())setBuyableAmount("ad",21,player.ad.points.div(1e32).log(1000).floor().add(1))
  ...
  getAllDimMult(){
    ...
    if(player.db.unlocked)s=s.mul(tmp.db.effect)
    return s.max(1)
  },
  ...
  if(hasUpgrade("ad",13))m=m.times(tmp.ad.upgrades[13].effect)
              ...
              if(player.b.clickables[12]==2)m=m.pow(1.2)
             return m.max(1)},
  update(diff){
    ...
    let d8mult = new Decimal(1)
    d8mult=d8mult.mul(buyableEffect("db",12))
    player.ad.d8=player.ad.d8.plus(diff.times(player.ad.d9).times(hasUpgrade("ad",11)&&!inChallenge("c",32)?Decimal.pow(1.1,getBuyableAmount("ad",33)):1).times(hasUpgrade("ad",23)&&!inChallenge("c",32)?Decimal.pow(1.025,getBuyableAmount("ad",33).times(9)):1).mul(player.e.sacrifice[4].pow(30).add(1)))
    player.ad.d7=player.ad.d7.plus(diff.times(player.ad.d8).times(hasUpgrade("ad",11)&&!inChallenge("c",32)?Decimal.pow(1.1,getBuyableAmount("ad",32)):1).times(hasUpgrade("ad",23)&&!inChallenge("c",32)?Decimal.pow(1.025,getBuyableAmount("ad",32).times(8)):1).times(d8mult))
    ...
    player.ad.d2=player.ad.d2.plus(diff.times(player.ad.d3.sqrt()).times(hasUpgrade("ad",11)&&!inChallenge("c",32)?Decimal.pow(1.1,getBuyableAmount("ad",13).sqrt()):1).times(hasUpgrade("ad",23)&&!inChallenge("c",32)?Decimal.pow(1.025,getBuyableAmount("ad",13).sqrt().times(3)):1))
    let d2m = new Decimal(1).times(player.ad.d2).times(hasUpgrade("ad",11)&&!inChallenge("c",32)?Decimal.pow(1.1,getBuyableAmount("ad",12)):1).times(hasUpgrade("ad",23)&&!inChallenge("c",32)?Decimal.pow(1.025,getBuyableAmount("ad",12).times(2)):1).times(hasChallenge("c",42)?Decimal.pow(1.5,getBuyableAmount("ad",13).sqrt()):1).div(player.ad.points.add(1).log10().max(1))
    if(d2m.gt(10)&&inChallenge("cc",12)){
      d2m=Decimal.pow(10,d2m.log10().pow(0.75))
    }
    if(!inChallenge("c",42))player.ad.d1=player.ad.d1.plus(d2m.times(diff))
    let d1m = new Decimal(1).times(player.ad.d1).times(hasUpgrade("ad",11)&&!inChallenge("c",32)?Decimal.pow(1.1,getBuyableAmount("ad",11)):1).times(hasUpgrade("ad",23)&&!inChallenge("c",32)?Decimal.pow(1.025,getBuyableAmount("ad",11)):1).times(layers.ad.getD1mult()).pow(inChallenge("c",22)?0.25:1)
    if(d1m.gt(10)){
      d1m=Decimal.pow(10,d1m.log10().pow(hasUpgrade("ad",25)?0.85:hasUpgrade("nerfs",22)?0.8:0.75))
    }
    if(hasChallenge("cc",12))d1m=d1m.mul(Decimal.pow(100,player.cc.challenges[12]))
    player.ad.points=player.ad.points.plus(d1m.times(diff))
...
addLayer("c", {
  ...
  resetsNothing(){return true},
    ...
    layerShown(){return player[this.layer].unlocked||hasUpgrade("ad",24)},
  update(diff){
...
canComplete(){return player.di.points.gte("1e3730")},
        unlocked(){return player.c.unlocked},
      goalDescription: "reach 1e3730 meters",
      rewardDescription: "Distance gain ^1.1",
    },
    21: {
        name: "Extreme",
        challengeDescription: "Time speed does nothing",
canComplete(){return player.di.points.gte("e1561")},
        unlocked(){return player.c.unlocked},
      goalDescription: "reach e1561 meters",
...
canComplete(){return player.di.points.gte("e4416")},
        unlocked(){return player.c.unlocked},
      goalDescription: "reach 1e4416 meters",
...
canComplete(){return player.ad.points.gte("e406")},
        unlocked(){return player.c.unlocked},
      goalDescription: "reach 1e406 antimatter",
...
canComplete(){return player.ad.points.gte("e249")},
        unlocked(){return player.c.unlocked},
      goalDescription: "reach 1e249 antimatter",
...
canComplete(){return player.ad.points.gte("e452")},
        unlocked(){return player.c.unlocked},
      goalDescription: "reach 1e452 antimatter",
...
layers: new Decimal(0),
  ...
  resetsNothing(){return true},
    ...
    layerShown(){return player[this.layer].unlocked||false},
  update(diff){
...
addLayer("xyz", {
  ...
  buyables: {
    11: {
        cost() { return new Decimal(1e8).times(Decimal.pow(100,getBuyableAmount(this.layer,this.id).pow(1.3))) },
...
if(!player.xyz.unlocked){return}
    ...
    if(player.b.clickables[13]==3)xyz=xyz.pow(tmp.b.effect)
    player.xyz.gps=xyz
...
12: {
  costBase(){
    ...
    if(player.b.clickables[13]==2)b=b.root(tmp.b.effect)
    return b
  },
        cost() {
  ...
  13: {
      ...
      effect(){
        ...
        canAfford() { return player[this.layer].trees.div(32).log(2).gte(player[this.layer].metatrees) },
      unlocked(){return hasUpgrade("ml",22)},
        buy() {
            player[this.layer].metatrees = player[this.layer].trees.div(32).log(2)
...
if(hasUpgrade("ml",11))player.ml.layers=player.ml.layers.plus(player.ml.points.pow(hasUpgrade("ml",15)?1.5:1).pow(hasUpgrade("ml",51)?1.69420:1).pow(hasUpgrade("ml",32)?2:1).pow(hasUpgrade("ml",33)?2:1).times(diff).times(hasUpgrade("ml",13)?upgradeEffect("ml",13):1).times(buyableEffect(this.layer,12)).times(hasUpgrade(this.layer,21)?player.l.points.plus(1).log10().plus(1).pow(hasUpgrade("ml",41)?player.ml.metatrees:1):1))
  },
  ...
  clickables: {
    11: {
      display() {return "Get someone to react 🆎 to this message"},
        canClick(){return player.ab.cooldown==0},
      onClick(){
        ...
        if(hasUpgrade("ab",24))gain=gain.times(3959)
        if(player.b.clickables[13]==1)gain=gain.pow(tmp.b.effect)
...
effect(){let f= player.b.best.div(100).add(1)
if(challengeCompletions("b",11)>=2)f=Decimal.pow(1.01,player.b.points)
return f},
  effectDescription(){return "Raising point gain to the "+format(this.effect())},
  branches:["ch"],
    color: "#cccccc",
    requires(){
  ...
  getNextAt(){return reqs[Number(player.b.points)]},
  canReset(){return player.points.gte(reqs[Number(player.b.points)])},
  prestigeButtonText(){return  "Reset all previous progress except minigames for 1 buff\nRequires "+format(this.getNextAt())+" points"},
  ...
  milestones: {
    ...
    6: {
        ...
        effectDescription: "Every 10 buffs, unlock another switch, each milestone multiplies energy by 1.2, and buy max AB buyables and they don't cost anything",
        done() { return player.b.best.gte(8) },
      unlocked(){return player.b.best.gte(7)},
    },
    7: {
        requirementDescription: "10 buffs",
        effectDescription: "double energy storage",
