...
function getTempCompCost() {
  ...
  return EN(20).times(EN(1.5).pow(game.tempComp.pow(1.5))).div(1+2*stinc(13)).div(stinc(13)?getSpaceEnergyRow1Mult():1).div(stinc(14)?2.25*(game.SEU.includes(9)?10:1):1).times(inGalChal(4)?10:1).pow(game.achievement.includes(43)&&game.inGalChal>=1?0.8:1)
}
...
function getSpaceCompCost(bulk=1) {
  return EN(0.2).times(EN(1.5).pow(game.spaceComp.add(bulk).minus(1).pow(1.5)))
    ...
    .pow(game.achievement.includes(43)&&game.inGalChal>=1?0.8:1)
}
...
function getSpaceTimeRate() {
  ...
  .times(game.achievement.includes(15)?2:1)
  .times(game.achievement.includes(31)?game.starTypes.add(1).pow(game.starTypes.add(1).log10().add(2)):1)
    ...
    .div(game.spaceless?(game.achievement.includes(32)? ExpantaNum(1e60).div(game.normalEnergy.max(1).log().add(1)):ExpantaNum(1e60)):1)
...
function getSpacetimeCompEffect() {
  return game.spacetimeComp
    .times(stinc(22)?EN(1).add(EN(0.015125).times(1+stinc(25)*(1+stinc(28))).times(game.spacetimeComp)):1)
...
function getTempCompEffect() {
  return getTempCompBase().pow(caplog10(game.tempComp).min(15).add(caplog10(getSpacetimeCompEffect())).add(inAnyGalChal()?0:game.superComp.temp)).times(game.achievement.includes(11)?(game.timeFoam.max(1).logBase(10).add(1).times(5)):1).times(game.achievement.includes(12)?(game.timeFoam.max(1).logBase(10000).add(1).times(5)):1).times(game.achievement.includes(13)?(game.timeFoam.max(1).logBase(Math.PI).add(1).times(Math.PI)):1).times(game.achievement.includes(15)?4:1).times(game.achievement.includes(31)?game.starTypes.add(1).pow(game.starTypes.add(1).log10().add(2)):1).times(game.achievement.includes(34)?game.galaxies[0].max(1).times(game.galaxies[1].max(1).times(game.galaxies[2].max(1).times(game.galaxies[3].max(1).times(game.galaxies[4].max(1).times(game.galaxies[5].max(1).times(game.galaxies[6].max(1).times(game.galaxies[7].max(1)))))))).pow(0.5):1)
}

function getTempCompBase() {
  return EN(1.1).add(stinc(12)?(stinc(15)?caplog10square(game.spaceComp):game.spaceComp.min(12)).times(0.015125):0).add(game.achievement.includes(28)?0.19:0)
}
...
function getSpaceCompBase() {
  ...
  return EN(1.1)
    .add(stinc(32)?game.tempComp.times(0.015125*(1+stinc(33))).times(stinc(33)?getSpaceEnergyRow1Mult():1):0)
