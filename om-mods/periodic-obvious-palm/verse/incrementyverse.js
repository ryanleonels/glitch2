...
They are based on your Singularity level.`
  ...
  get("fractalBoostsText").innerHTML=`<b>You have ${getFractalBoost()} Fractal Boost${(getFractalBoost()==1?"":"s")}<br>providing a ${EN(2).pow(getFractalBoost())}x speed</b>`
}
...
function updateFractalEngine(ms) {
  ...
  for (let ind=game.fractalEngine.length-1;ind>=0;ind--) {
    ...
    mach.charge=mach.charge.add(tempown.times(ms/1000).times(getMachMult(mach.level).mul(1e17).mul(EN(2).pow(getFractalBoost()))))
...
function getFractalBoost(x=game.fractalUpgrades[5]) {
    return x
}
...
function fuep(x,spectate=0) {
  ...
  let cost = [2,3,5,10,20,10][x-1]+[1,2,3,5,10,10][x-1]*game.fractalUpgrades[x-1]
