...
class SingularityFunction {
  ...
  canBuy() {
    return (this.id==11||(this.id<=25&&getBaseless()>=3)||getSingLevel()+game.manifolds-game.sing.m-game.spentFunctions >= this.price && !(game.sfBought.indexOf(this.id) > -1) && this.prereq.some(id => {return game.sfBought.includes(id)}) && game.sfBought.indexOf(this.splitpath[0]) == -1 && game.sfBought.indexOf(this.splitpath[1]) == -1 && this.specialReq())
  }
}
