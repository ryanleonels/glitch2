...
function calcDynamic() {
  return (
    ...
    (game.upgrades.includes(13) && (inChal(2)||inChal(4)||inChal(6)||inChal(8)) ? 2 : 1)
  );
}
...
function getDynamicFactorCap() {
  return (
    ...
    (game.upgrades.includes(13) && (game.challenge % 2 === 1 || game.challenge2[0] % 2 === 1 || game.challenge2[1] % 2 === 1) ? 2 : 1)
  );
}
...
function getIncrementyRate(x) {
  ...
  return EN(
    ((ordRate * x) / 1000) *
      (game.omegaChallenge == 2?1:double()) ** game.iups[1] *
...
function double() {
  return Math.max(2+getOCComp(2),2*getOCComp(2))
}
