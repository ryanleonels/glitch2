...
const baselessMile = [5**75,5**90,5**115,Infinity]
let ordColor = "no";
const EN = ExpantaNum;
...
function loop(unadjusted, off = 0) {
  let ms=unadjusted
  if (inChal(8)&&game.decrementy<10) {
    ms=50
  }
  ...
  if (getOCComp(1)>=1) {
    ...
    game.chal8Comp=Math.min(16,Math.max(game.chal8Comp,Math.floor((getOCComp(1)**1.4)*3)))
  }
  ...
  if ((game.challenge>0) && !(game.chal8===1) && game.leastBoost <= 1.5 && game.qolSM.acc === 1) {
    if ( // !(1) = false iirc
  ...
  if (game.chal8 === 1 && calcRefund() > 0) // no touch this one
    confirm("You failed Challenge 8 because you had booster upgrades on you!");
  if ((game.chal8 === 1) && calcRefund() > 0) refund(); // no touch this, softlocks
  game.boosters =
  ...
  for (assCount in game.assCard) {
    ...
    game.assCard[assCount].mult = game.assCard[assCount].power
      ...
      .pow(Math.max(1,Math.sqrt(getOCComp(5))));
  ...
  if (ms > 0) {
    if ((game.upgrades.includes(2) || game.leastBoost <= 1.5) && game.autoOn.max === 1) {
      ...
      if (game.bAutoLoop.max >= 1000 / buptotalMute) {
        ...
        if ((game.OP < (inChal(5) && game.factorShifts >= 2 ? Infinity : getFSCost(game.factorShifts)) && ((!(game.challenge > 0) && !(game.chal8)) || game.OP < (challengeGoals[game.challenge - 1] == undefined ? 1e300 : challengeGoals[game.challenge-1][game.challengeCompletion[game.challenge - 1]]))) || game.OP >= 1e265) {
          maxInfStuff();
        }
      }
    }
...
function render() {
  ...
  game.canInf =
    ...
    outSize >= Infinity;
  get("psiLevel").innerText = getPsi(game.ord)
  if (game.infUnlock === 1) {
  ...
  get("dynamicMult").textContent =
    ...
    (
      ...
      (game.upgrades.includes(13) && (inChal(1)||inChal(3)||inChal(5)||inChal(7)) ? 2 : 1)
    ).toFixed(3);
  ...
  get("iup2").innerHTML =
    ...
    beautify(10 ** (3 *((game.omegaChallenge==4)+1)* (game.iups[1] + 1)));
  get("iup3").innerHTML =
  ...
  get("manifoldIncrease").textContent =
    "It is increasing by " +
    (game.upgrades.includes(13) && (inChal(1)||inChal(3)||inChal(5)||inChal(7)) // tip: use ctrl+f
...
They are based on your Singularity level.`
  ...
  get("ocFBpsReq").textContent=(game.bestFBps<5e8?"500,000,000":"Infinity")
  get("oc4Effect").textContent=(1/(1+getOCComp(4))).toFixed(6)
  get("oc5Effect").textContent=(Math.max(1,Math.sqrt(getOCComp(5)))).toFixed(6)
}
...
function iup(n, spectate = 0) {
  ...
  if (n <= 3) {
    ...
    if (game.incrementy.gte(iupCosts[n - 1] ** ((game.iups[n - 1] + 1)*((n==2&&game.omegaChallenge==4)+1)))) {
      if (spectate == 0) {
        game.iups[n - 1] += 1;
        game.incrementy = game.incrementy.minus(
          iupCosts[n - 1] ** (game.iups[n - 1]*((n==2&&game.omegaChallenge==4)+1))
        );
      } else {
        get("iup" + n).classList.add("boosterButton");
      }
...
function csubTab(t) {
  [1,2,3,4,5,6,7,8].forEach(i => get("csubTab"+i).style.display = "none")
