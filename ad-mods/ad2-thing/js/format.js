function getFullExpansion(num, places) {
	num = new Decimal(num);
	if(num.gt(1e4)) return shorten(num);
...
function shorten(num, places = 2, shortPlaces = 2) {
	...
	if(num.iteratedlog(10, 2).gt(1e15) && (game.options.notation == "Logarithmic" || game.options.notation == "Infinitic")) {
		var end = getFullExpansion(num.layer+1);
		return "E" + Math.log10(num.mag).toString().substring(0, Math.max(7 - end.length, 4)) + "#" + end;
	}
	...
	if(game.options.notation == "Scientific" || (game.options.notation == "Mixed Scientific" && num.gt(game.options.mixedCutoff))) {
		...
		if(num.lt("ee15")) return num.mantissa.toFixed(places) + "e" + getFullExpansion(num.exponent);
		return "ee" + getFullExpansion(num.log10().log10(), 6);
	}
	if(game.options.notation == "Logarithmic") {
		return Logarithmic(num, shortPlaces);
	}
	if(game.options.notation == "Infinitic") {
		return InfinityPercent(num, shortPlaces);
	}
	else if(game.options.notation == "Standard" || game.options.notation == "Mixed Scientific") {
		return toStandard(num, places, shortPlaces)
	}
	if(game.options.notation == "Letter") {
		return toLetter(num, places, shortPlaces);
	}
	if(game.options.notation == "Cancer") {
		return toCancer(num, places, shortPlaces);
	}
}
...
function Logarithmic(num, shortPlaces) {
		if(num.lt(1e3)) return num.toFixed(shortPlaces);
		if(num.lt("ee6")) return "e" + getFullExpansion(num.log10(), 2);
...
function InfinityPercent(num, shortPlaces) {
    ...
    const pow = Decimal.log10(num)
    var reduced = pow / inflog
    if (reduced < 1000) var infPlaces = 4
     else var infPlaces = 3
		if(num.lt("ee12")) return getFullExpansion(reduced, infPlaces) + '∞';
		return InfinityPercent(Decimal.log10(reduced).divide(inflog), infPlaces) + '∞';;
}
...
function toLetter(value, places, shortPlaces) {
		var mantissa=value.m
		var exponent=value.e
		mantissa=mantissa*Math.pow(10,exponent%3)
		exponent=exponent-exponent%3
	  let prefixes = ['a','b','c','d','e','f',
		...
		if (exponent<3) return mantissa.toFixed(shortPlaces)
		else if (exponent<78) return mantissa.toFixed(places)+" "+prefixes[exponent/3]
		else {
			var result = ''
			exponent = Math.floor(exponent/3)-1;
			e2 = 0
			while (exponent > 0) {		
				...
				if (partE > 0) {
					if (partE == 1 && e2 > 0) var prefix = ""
					else var prefix = prefixes[Math.floor(partE/26) % 26] + prefixes[partE % 26]
...
function toCancer(value, places, shortPlaces) {
		var mantissa=value.m
		var exponent=value.e
		mantissa=mantissa*Math.pow(10,exponent%3)
		exponent=exponent-exponent%3
