...
function shorten(num, places = 2, shortPlaces = 2) {
  ...
  if(game.options.notation == "Logarithm") {
	...
	else if(game.options.notation == "Cancer") {
		return cancer(num, places, shortPlaces)
	}
}
...
function cHeroes(num, places, shortPlaces) {
  ...
  else if(num.lt(Decimal.pow(10, cPrefixes.length+1))) return mantissa.toFixed(places)+cPrefixes[(exponent/3)-1]
  else {
    function compact(exponent) {
      if (exponent < 3) return '';
      return cPrefixes[Math.floor(exponent/3)%(cPrefixes.length)] + '' + compact((exponent/3)-1)
    }
    return mantissa.toFixed(places)+compact(exponent)
  }
}

function cancer(num, places, shortPlaces) {
		var mantissa=num.m
		var exponent=num.e
		mantissa=mantissa*Math.pow(10,exponent%3)
		exponent=exponent-exponent%3
